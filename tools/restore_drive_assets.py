...
def download(file_id: str, destination: pathlib.Path) -> None:
    ...
    if width < 64 or height < 64:
        raise RuntimeError(f"Suspiciously small PNG for {destination}: {width}x{height}")
    destination.write_bytes(data)
    print(f"restored Drive PNG {destination.relative_to(ROOT)} ({len(data)} bytes, {width}x{height})")
...
def patch_combat() -> None:
    ...
    for old, new in replacements.items():
        text = replace_once_if_needed(text, old, new)
